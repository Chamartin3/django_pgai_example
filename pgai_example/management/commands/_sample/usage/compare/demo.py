"""Demo command - regenerates DEMONSTRATION.md at the repo root.
...
OUTPUT_PATH = Path("DEMONSTRATION.md")
...
def generate(queries: list[str], limit: int, runs: int) -> str:
    """Build the full DEMONSTRATION.md content."""
    parts: list[str] = []
    parts.append("# DEMONSTRATION.md\n")
    parts.append(
    ...
    parts.append(
        "The example ships with two ways to monitor embedding progress while "
        "the `vectorizer-worker` is running.\n\n"
        "**Django admin** — the plugin registers a `Vectorizer` admin view with "
        "state badges (`queued` / `active` / `done`), progress bars, pending "
        "counts, and source/embedding-store columns. Enable in `INSTALLED_APPS` "
        "via the plugin's `register_admin` opt-in.\n\n"
        "![Django admin: vectorizer list](docs/images/admin-vectorizers.png)\n\n"
        "**`pgai list` command** — terminal view from the plugin with per-"
        "vectorizer state, throughput (chunks/s), batch size, and ETA. Run it "
        "to watch a backfill complete:\n\n"
        "```bash\n./manage.sh pgai list\n```\n\n"
        "![pgai list terminal output](docs/images/pgai-list.png)\n\n"
        "Use either one to confirm all four vectorizers reach `done` before "
        "running the queries below — partial vectorization will skew the "
        "comparison.\n"
    )
...
class DemoCommand:
    ...
    def demo(
        ...
        """Run canonical queries across all variants; write DEMONSTRATION.md."""
