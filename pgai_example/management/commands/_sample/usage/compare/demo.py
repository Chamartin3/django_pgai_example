"""Demo command - canonical query set across all vectorizer variants.

Runs a curated query set against every variant, then writes both a
markdown report and a sibling JSON file with the raw measurements to
``$BENCHMARKS_DIR/usage.{md,json}``. The JSON is the canonical artifact
used to update the handwritten docs under ``docs/``.
"""
...
def generate(
    ...
    If ``raw`` is provided, per-query results/timings are appended to it so the
    caller can persist the raw measurement data separately from the markdown
    rendering.
    """
    parts: list[str] = []
    parts.append("# Benchmark report — `usage compare demo`\n")
    parts.append(
        f"Generated: `{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}` — "
        "raw measurement data for the same run lives in `usage.json`.\n"
    )
    parts.append("## Setup\n")
    parts.append(
...
def handle_demo(queries: list[str], limit: int, runs: int, output: Path) -> None:
    try:
        ...
        content = generate(queries=queries, limit=limit, runs=runs, raw=raw)
        ...
        ctx.render.message.success(
            f"Wrote {output} ({len(content):,} bytes) and {json_path}"
        )
...
class DemoCommand:
    ...
    def demo(
        ...
        """Run canonical queries across all variants; write benchmark report."""
        query_list = [q.strip() for q in queries.split(",") if q.strip()]
        handle_demo(
            queries=query_list, limit=limit, runs=runs, output=output,
        )
